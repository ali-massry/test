...
class SummaryOfPnflOperator extends StatefulWidget {
  const SummaryOfPnflOperator({super.key});

  @override
  State<SummaryOfPnflOperator> createState() => _SummaryOfPnflOperatorState();
}

class _SummaryOfPnflOperatorState extends State<SummaryOfPnflOperator> {
                        ...
                        children: [
                                  ...
                                  itemBuilder: (context, index) {
                                    return GestureDetector(
                                      ...
                                      child: BuildCategoryEditCard(
                                        title:
                                            '${context.read<ForestCubit>().forestessences?.name} | ${context.read<ForestCubit>().filterAllForestsAccordingToLinkId(linkId: 4)[index].prodid}',
                                        columnWidget: Column(
                                          crossAxisAlignment:
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              '${context.read<ForestCubit>().forestessences?.name} | ${context.read<ForestCubit>().productNameFromId(context, productId: int.parse(context.read<ForestCubit>().filterAllForestsAccordingToLinkId(linkId: 4)[index].prodid.toString()))}',
                                              style: GlobalTextStyles
                                                  .ts15montboldBlack,
                                            ),
                                            SizedBox(height: mqh(context, 15)),
                                            Text(
                                              '${context.read<ForestCubit>().filterAllForestsAccordingToLinkId(linkId: 4)[index].qty} / ${context.read<ForestCubit>().unitNameFromId(context, unitId: int.parse(context.read<ForestCubit>().filterAllForestsAccordingToLinkId(linkId: 4)[index].unitid.toString()))}',
                                              style: GlobalTextStyles
                                                  .ts13montsemiBold30OpaquBlack,
                                            ),
                                            SizedBox(height: mqh(context, 8)),
                                            Text(
                                              '${context.read<ForestCubit>().filterAllForestsAccordingToLinkId(linkId: 4)[index].annualprodfcfa} FCFA',
                                              style: GlobalTextStyles
                                                  .ts13montsemiBold30OpaquBlack,
                                            ),
                                            SizedBox(height: mqh(context, 20)),
                          ...
                          Column(
                            children: [
                              ...
                              GestureDetector(
                                onTap: () {
                                  context.read<ForestCubit>().showBackInPNFL =
                                      false;
                                      context
                                      .read<ForestCubit>()
                                ...
                                child: Center(
                                  child: Text(
                                    'Ajouter une autre essence forestière non-ligneuse exploitée'
                                        .toUpperCase(),
                                    style: GlobalTextStyles
